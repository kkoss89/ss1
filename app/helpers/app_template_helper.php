    ...
    function show_item_button_action($controller_name, $ids, $format = 'dropdown', $item_data = [], $params = []) {
        ...
        switch ($format) {
            ...
            default:
				...
				if(isset($item_data['is_interval_order']) && $item_data['is_interval_order']){
					$xhtml .= '<a href="'.base_url('admin/order/get_sub_orders/'.$item_data['id']).'" class="dropdown-item ajaxSubOrders" data-confirm_ms=""><i class="dropdown-icon fe fe-eye"></i> Sub Orders</a>';
				}
				
				if(isset($item_data['comments_enabled']) && $item_data['comments_enabled']){
					$xhtml .= '<a href="'.base_url('admin/services/comments/'.$item_data['id']).'" class="dropdown-item"><i class="dropdown-icon fa fa-comment"></i> Add Comments</a>';
				}
...
if (!function_exists('show_item_order_details')) {
    ...
    function show_item_order_details($controller_name, $item = [], $params = [], $task = 'admin') {
        ...
        switch ($controller_name) {
            ...
            case 'refill':
                $order_attrs = [
                    ...
                    'start_counter' => ($item['start_counter']) ? $item['start_counter'] : 0,
                ];
                break;
            ...
            default:
				...
				if ($task == 'suborders') {
					...
					$order_attrs = [
						...
						'charge' => $charge,
						'start_counter' => ($item['start_counter']) ? $item['start_counter'] : 0,
						'remain' => ($item['remains']) ? $item['remains'] : 0,
					];
				}
                break;
        }
        ...
        $xhtml_order_attr .= '</ul>';
        $service_name = '<span class="badge bg-primary">' . $item['service_id'] . '</span> ' . $category_name . ' - ' . $item['service_name'];
        ...
        switch ($controller_name) {
            ...
            case 'refill':
                $order_attrs = [
                    ...
                    'start_counter' => ['name' => lang('Start_counter'), 'value' => ($item['start_counter']) ? $item['start_counter'] : 0],
                ];
                break;
            ...
            default:
                ...
                $order_attrs = [
                    ...
                    'start_counter' => ['name' => lang('Start_counter'), 'value' => ($item['start_counter']) ? $item['start_counter'] : 0],
                    'remain' => ['name' => lang('Remains'), 'value' => ($item['remains']) ? $item['remains'] : 0],
                ];
                break;
        }
...
if (!function_exists('show_item_order_id')) {
    ...
    function show_item_order_id($controller_name, $item = [], $params = []) {
		if(isset($item['order_id']) && $item['order_id'] != ''){
			$xhtml = null;
			$order_id = show_high_light($item['order_id'], $params['search'], 'order_id');
			$api_order_id = '';
