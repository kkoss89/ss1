...
if (!function_exists('show_page_header')) {
    ...
    {
		$sub_page_title = '';
		if(($params['sub_page_title']) && $params['sub_page_title'] != ''){
        ...
        switch ($params['page-options']) {
            case 'add-new':
                ...
                break;
			 case 'add-new-service-comment':
                if (staff_has_permission($controller_name, 'add')) {
                    # code...
                    $add_new_link = admin_url($controller_name . "/update_service_comment/".$params['service_id']);
                    $xhtml_page_options = sprintf(
                        '<div class="d-flex">
                            <a href="%s" class="ml-auto btn btn-outline-primary %s">
                                <span class="fe fe-plus"></span>
        ...
        $xhtml = sprintf(
            ...
            </div>', $current_controller['icon'], $current_controller['name'].$sub_page_title, $xhtml_page_options
        );
        return $xhtml;
    }
}
