...
class auth extends My_UserController {
    ...
    public function login() {
        ...
        if (isset($_POST['g-recaptcha-response']) && get_option("enable_goolge_recapcha", '') && get_option('google_capcha_site_key') != "" && get_option('google_capcha_secret_key') != "") {
            ...
            if (!$resp->isSuccess())
                _validation('error', 'Please verify recaptcha');
        }
    }
    ...
    public function ajax_sign_in() {
		...
		$admin = $this->model->get("id, status, ids, email, password, first_name, last_name, timezone", $this->tb_staff, ['email' => $email]);
		...
		if($admin){
            ...
            if ($admin && $this->model->app_password_verify(post("password"), $admin->password)) {
                $error = false;
            } else {
                $error = true;
            }
            ...
            if (!$error) {
                ...
                $this->db->update($this->tb_staff, $item_update, ['id' => $admin->id]);
                
				ms(array(
					"status"   => "success",
					"message"  => "Login successfully",
					"redirect" => cn('admin/statistics')
				));
            } else {
				ms(array(
					"status" => "error",
					"message" => "Email address and password that You entered doesn't match any account. Please check your account again",
				));
            }
		} else {
			$error = false;
